...
const Desktop = () => {
  ...
  const [minTextLeft, setMinTextLeft] = useState(450);
  const [maxTextLeft, setMaxTextLeft] = useState(0);
  const [heightTextTR, setHeightTextTR] = useState(40);
  const [widthTextTR, setWidthTextTR] = useState(0);

  useEffect(() => {
    if (!refMinTextLeft.current || !refMaxTextLeft.current) return;
    setMinTextLeft(refMinTextLeft.current?.offsetWidth);
    setMaxTextLeft(refMaxTextLeft.current?.offsetWidth);
  }, [refMinTextLeft.current, refMaxTextLeft.current]);

  useEffect(() => {
    const resizeObserver = new ResizeObserver((entries) => {
      for (let entry of entries) {
        const { width, height } = entry.contentRect;
        setWidthTextTR(width);
        setHeightTextTR(height);
      }
    });

    resizeObserver.observe(refTextTR.current as Element);
